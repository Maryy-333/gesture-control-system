...
class TestFirstGestureRequiresStability:
    @pytest.mark.parametrize("gesture", [Gesture.POINT, Gesture.FIST, Gesture.PEACE])
    def test_first_frames_hold_initial_state_then_confirm(self, gesture: Gesture) -> None:
...
class TestFlickerDoesNotChangeStableGesture:
    def _seed(self, smoothener: GestureSmoothener, gesture: Gesture = Gesture.POINT) -> None:
        for _ in range(3):
    ...
    def test_isolated_flicker_is_absorbed(self) -> None:
        ...
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
    ...
    def test_non_consecutive_alternation_never_confirms(self) -> None:
        smoothener = GestureSmoothener(stability_frames=3)
        self._seed(smoothener)
        results = [smoothener.smooth(g) for g in (Gesture.FIST, Gesture.POINT, Gesture.FIST, Gesture.POINT)]
        assert results == [Gesture.POINT, Gesture.POINT, Gesture.POINT, Gesture.POINT]
        assert smoothener.stable_gesture == Gesture.POINT
...
class TestConsecutiveTransition:
    def test_three_consecutive_frames_confirm_new_gesture(self) -> None:
        ...
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.FIST) == Gesture.FIST
        assert smoothener.stable_gesture == Gesture.FIST
    ...
    def test_spec_example_A_full_sequence(self) -> None:
        ...
        outputs = [
            smoothener.smooth(g)
            for g in (
                Gesture.POINT, Gesture.POINT, Gesture.FIST, Gesture.POINT,
                Gesture.FIST, Gesture.FIST, Gesture.FIST,
            )
        ]
        assert outputs == [
            Gesture.POINT, Gesture.POINT, Gesture.POINT, Gesture.POINT,
            Gesture.POINT, Gesture.POINT, Gesture.FIST,
        ]
...
class TestCandidateCounterResets:
    def test_candidate_run_is_broken_by_a_different_gesture(self) -> None:
        # Stable POINT; two FIST frames (candidate_count: 1, 2), then a
        # POINT breaks the run; then three fresh consecutive FIST frames
        # finally confirm FIST.
        smoothener = GestureSmoothener(stability_frames=3)
        for _ in range(3):
            smoothener.smooth(Gesture.POINT)
        assert smoothener.stable_gesture == Gesture.POINT
        ...
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.POINT) == Gesture.POINT  # breaks the run

        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.FIST) == Gesture.POINT
        assert smoothener.smooth(Gesture.FIST) == Gesture.FIST

    def test_candidate_gesture_different_from_stable_does_not_accumulate(self) -> None:
        # A FIST then a PEACE (different from both stable POINT and the
        # FIST candidate) resets the candidate to PEACE; then FIST again
        # starts a fresh candidate run. Neither run reaches 3, so stable
        # stays POINT.
        smoothener = GestureSmoothener(stability_frames=3)
        for _ in range(3):
            smoothener.smooth(Gesture.POINT)
...
class TestNoDelayForStableGesture:
    def test_holding_stable_gesture_reports_immediately(self) -> None:
        smoothener = GestureSmoothener(stability_frames=3)
        outputs = [smoothener.smooth(Gesture.POINT) for _ in range(4)]
...
class TestReset:
    ...
    def test_reset_clears_candidate_state(self) -> None:
        ...
        for _ in range(3):
            smoothener.smooth(Gesture.POINT)
        smoothener.smooth(Gesture.FIST)
        smoothener.smooth(Gesture.FIST)
        smoothener.reset()
        assert smoothener.stable_gesture is None
        assert smoothener.smooth(Gesture.POINT) == Gesture.POINT
...
class TestInstanceIndependence:
    ...
    def test_different_stability_frames_are_independent(self) -> None:
        ...
        assert fast.smooth(Gesture.FIST) == Gesture.FIST
        assert slow.smooth(Gesture.FIST) == Gesture.POINT
...
class TestSpecExamples:
    def test_example_b(self) -> None:
        ...
        for _ in range(3):
            smoothener.smooth(Gesture.POINT)
        outputs = [smoothener.smooth(Gesture.FIST) for _ in range(3)]
        assert outputs == [Gesture.POINT, Gesture.POINT, Gesture.FIST]
    ...
    def test_example_c(self) -> None:
        ...
        outputs = [
            smoothener.smooth(g)
            for g in (Gesture.FIST, Gesture.POINT, Gesture.FIST, Gesture.POINT)
        ]
        assert outputs == [Gesture.POINT] * 4