...
class TestDiscreteActionDebouncing:
    def test_first_occurrence_of_a_gesture_fires(
    ...
    def test_returning_to_a_previous_gesture_after_switching_away_fires_again(
        ...
        assert gate.should_execute(
            Gesture.THREE_FINGERS, Action.LEFT_CLICK
        ) is True  # THREE_FINGERS is "new" again
...
class TestContinuousActionsAlwaysFire:
    def test_move_cursor_fires_every_frame_while_held(
        self, gate: GestureActionGate
    ) -> None:
        for _ in range(10):
            assert gate.should_execute(
    ...
    def test_scroll_down_fires_every_frame_while_held(
        self, gate: GestureActionGate
    ) -> None:
        for _ in range(10):
            assert gate.should_execute(
    ...
    def test_scroll_up_fires_every_frame_while_held(
        self, gate: GestureActionGate
    ) -> None:
        for _ in range(10):
            ...
            assert gate.should_execute(
                Gesture.UNKNOWN, Action.SCROLL_UP
            ) is True
...
class TestNoHandInput:
    def test_none_gesture_and_action_returns_false(
        self, gate: GestureActionGate
    ) -> None:
    ...
    def test_losing_the_hand_does_not_resume_a_paused_gate(
        self, gate: GestureActionGate
    ) -> None:
...
class TestPauseToggle:
    ...
    def test_holding_open_palm_does_not_re_toggle(
        self, gate: GestureActionGate
    ) -> None:
        gate.should_execute(Gesture.OPEN_PALM, Action.PAUSE)
        assert gate.is_paused is True
    ...
    def test_second_distinct_open_palm_entry_resumes(
        self, gate: GestureActionGate
    ) -> None:
        gate.should_execute(Gesture.OPEN_PALM, Action.PAUSE)
        assert gate.is_paused is True
    ...
    def test_pause_never_dispatches_even_though_it_toggles_state(
        ...
        # PAUSE itself is always False; only the THREE_FINGERS in between
        # reflects suppression while paused.
...
class TestSuppressionWhilePaused:
    def test_discrete_actions_suppressed_while_paused(
        self, gate: GestureActionGate
    ) -> None:
        gate.should_execute(Gesture.OPEN_PALM, Action.PAUSE)
    ...
    def test_dispatch_resumes_normally_after_unpausing(
        self, gate: GestureActionGate
    ) -> None:
        gate.should_execute(Gesture.OPEN_PALM, Action.PAUSE)
...
class TestReset:
    def test_reset_re_arms_discrete_actions(
        self, gate: GestureActionGate
    ) -> None:
        gate.should_execute(
            Gesture.THREE_FINGERS, Action.LEFT_CLICK
...
class TestDeterminism:
    ...
    def test_two_gate_instances_do_not_share_state(self) -> None:
        ...
        gate_a.should_execute(
            Gesture.THREE_FINGERS, Action.LEFT_CLICK
        )

        assert gate_a.should_execute(
            Gesture.THREE_FINGERS, Action.LEFT_CLICK
        ) is False

        # gate_b has never seen THREE_FINGERS -- it must fire fresh.
        assert gate_b.should_execute(
            Gesture.THREE_FINGERS, Action.LEFT_CLICK
        ) is True
